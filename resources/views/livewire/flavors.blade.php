                            ...
                            <div class="border-t border-gray-200 pb-4 pt-4">
                                <fieldset>
                                    ...
                                    <button type="button" class="flex w-full items-center justify-between p-2 text-gray-400 hover:text-gray-500" aria-controls="filter-section-0" aria-expanded="false">
                                        <span class="text-sm font-medium text-gray-900">Flavors</span>
                                    </button>
                                    </legend>
                                    ...
                                    <div class="space-y-6">
                                        @foreach($flavors as $flavor)
                                            <div
                                                ...
                                                class="flex items-center p-2 {{ !empty($hoveredFlavor) && $flavor->id == $hoveredFlavor->id ? 'bg-gray-100' : '' }} " ">
                                                {{ $flavor->flavor }}
                                            </div>
                                        @endforeach
                                    </div>
                    ...
                    <aside>
                        ...
                        <div class="hidden lg:block">
                            <form class="space-y-10 divide-y divide-gray-200">
                                <div class="pt-4">
                                    <fieldset>
                                        <legend class="block text-sm font-medium text-gray-900">Flavors</legend>

                                        <div class="space-y-3 pt-4">
                                            @foreach($flavors as $flavor)
                                                <div
                                                    wire:mouseover="setHoveredFlavor({{ $flavor->id }})"
                                                    class="flex items-center hover:bg-gray-100 p-2 {{ !empty($hoveredFlavor) && $flavor->id == $hoveredFlavor->id ? 'bg-gray-100' : '' }} ">
                                                    <label for="sizes-0" class="ml-3 text-sm text-gray-600">{{ $flavor->flavor }}</label>
                                                </div>
                                            @endforeach
                                        </div>
                                    </fieldset>
                    ...
                    <div class="lg:col-span-2 lg:mt-0 xl:col-span-3">
                        <div class="bg-white">
                            <div class="mx-auto max-w-2xl px-4 py-16 sm:px-6 sm:py-4 lg:max-w-7xl lg:px-8">
                                <h2 class="text-xl mb-2 font-bold text-gray-900">{{ empty($hoveredFlavor) ? '' : $hoveredFlavor->flavor }}</h2>

                                <div class="mt-4 grid grid-cols-1 gap-y-12 sm:grid-cols-2 sm:gap-x-6 lg:grid-cols-3 xl:gap-x-8">
                                    @if(!empty($hoveredFlavor))
                                        @foreach($hoveredFlavor->products as $product)
